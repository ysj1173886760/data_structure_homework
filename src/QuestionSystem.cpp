...
bool QuestionSystem::CheckKeyWord(const std::string& s, const std::vector<std::string>& v) {
    BasicOperation op;
    for(int j=0; j<v.size(); j++) {
        double sim = op.GetWordsSim(s, v[j]);
        if (sim > 0.6)
            return true;
    }
    return false;
...
std::string QuestionSystem::GetAnswer(const std::string& id, const std::string& question) {
    ...
    if(CheckKeyWord("phone", word))
        return seller.shop_owner_phone_number;
    else if(CheckKeyWord("address", word))
    ...
    for(int i=0; i<sel_items.size(); i++) {
        if(CheckKeyWord(sel_items[i].name, word)) {
            if(CheckKeyWord("price", word))
                return to_string(sel_items[i].price);
            else if(CheckKeyWord("description", word))
                return sel_items[i].des;
            else if(CheckKeyWord("sales", word))
